        ...
        <div
          className={`
            ...
            ${isUser
              ...
              : isError
                ? 'bg-red-50 border border-red-200 text-red-800 rounded-3xl rounded-tl-sm'
        ...
        >
          {isUser ? (
            ...
            <div className="text-[15px] leading-relaxed markdown-body
              [&_pre]:my-4 [&_pre]:p-4 [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:bg-slate-900 [&_pre]:text-slate-50 [&_pre]:shadow-sm
              ...
              [&_code:not(pre_code)]:px-1.5 [&_code:not(pre_code)]:py-0.5 [&_code:not(pre_code)]:mx-0.5 [&_code:not(pre_code)]:rounded-md [&_code:not(pre_code)]:bg-slate-100 [&_code:not(pre_code)]:text-slate-700 [&_code:not(pre_code)]:font-mono [&_code:not(pre_code)]:text-[13.5px] [&_code:not(pre_code)]:font-bold [&_code:not(pre_code)]:border [&_code:not(pre_code)]:border-slate-200
            ">
              <ReactMarkdown
                ...
                components={{
                  ...
                  h1: ({node, ...props}) => <h1 className="text-xl font-extrabold text-slate-900 mb-3 mt-5 tracking-tight" {...props} />,
                  h2: ({node, ...props}) => <h2 className="text-lg font-extrabold text-slate-800 mb-2 mt-4 tracking-tight" {...props} />,
                  h3: ({node, ...props}) => <h3 className="text-base font-extrabold text-slate-800 mb-2 mt-3 tracking-tight" {...props} />,
                  strong: ({node, ...props}) => <strong className="font-bold text-slate-900" {...props} />,
                  blockquote: ({node, ...props}) => <blockquote className="border-l-4 border-slate-300 pl-4 py-1 my-3 bg-slate-50 text-slate-700 italic rounded-r-lg" {...props} />,
                  a: ({node, ...props}) => <a className="text-blue-700 hover:text-blue-900 hover:underline font-bold transition-colors" target="_blank" rel="noopener noreferrer" {...props} />
                }}
              >
                {message.content}
              </ReactMarkdown>
          ...
          {!isUser && riesgo && (
            <div className="mt-4 p-3.5 bg-amber-50 border border-amber-200 rounded-2xl text-sm flex gap-3">
              <span className="text-amber-600 shrink-0 text-base">⚠️</span>
              <div>
                <span className="font-bold text-amber-900 text-xs uppercase tracking-wider">Alerta académica</span>
                <p className="text-amber-800 mt-0.5 text-xs leading-relaxed font-medium">
          ...
          {!isUser && Array.isArray(fragmentos) && fragmentos.length > 0 && (
            <div className="mt-4 pt-3 border-t border-slate-100 text-xs">
              <details className="group">
                <summary className="cursor-pointer text-slate-500 hover:text-slate-800 font-bold transition-colors flex items-center gap-1.5 select-none outline-none text-[11px]">
                ...
                <div className="mt-3 space-y-2 pl-5">
                  {fragmentos.map((f, idx) => (
                    <div key={idx} className="p-3 bg-[#FAF9F6] border border-slate-100 rounded-xl text-slate-600 italic leading-relaxed text-[11px]">
