    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">Revisa y aprueba los sílabos subidos por estudiantes que requieren validación manual.</p>
        </div>
        <Button onClick={loadPendingSyllabi} variant="outline" className="flex items-center gap-2 shadow-sm bg-white">
          <RefreshCw className="w-4 h-4" /> Recargar
        </Button>
      </div>

      {syllabi.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-3xl shadow-sm p-16 text-center">
          <div className="w-20 h-20 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle2 className="w-10 h-10 text-emerald-500" />
          </div>
          <h2 className="text-xl font-bold text-slate-800 mb-2">Bandeja de Entrada Limpia</h2>
        ...
        <div className="grid grid-cols-1 gap-5">
          {paginatedSyllabi.map((syllabus) => (
            <div key={syllabus.id_silabo} className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm hover:shadow-md transition-all flex flex-col md:flex-row md:items-start justify-between gap-6 group">
              <div className="flex-1 space-y-4">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-bold text-[10px] uppercase tracking-wider bg-slate-100 text-slate-600 px-2 py-1 rounded-md border border-slate-200 flex items-center gap-1.5">
                    <FileText className="w-3.5 h-3.5" /> {syllabus.codigo_curso || 'N/A'}
                  </span>
                  <span className="font-bold text-[10px] uppercase tracking-wider bg-indigo-50 text-indigo-600 px-2 py-1 rounded-md border border-indigo-100 flex items-center gap-1.5">
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-y-2 gap-x-5 text-xs text-slate-500">
                      <p className="flex items-center gap-1.5">
                        <span className="font-semibold text-slate-400">SUBIDO POR:</span> 
                        <span className="font-bold text-slate-700">{syllabus.usuario_nombre}</span>
                      </p>
                      <p className="flex items-center gap-1.5">
                  ...
                  <div className="flex items-center gap-3">
                    <Button
                      ...
                      className="rounded-xl border-slate-200 text-indigo-600 hover:bg-indigo-50 px-3 py-1 text-xs flex items-center gap-2 shadow-sm bg-white"
                    >
                    ...
                    {syllabus.ruta_pdf && (
                      <a 
                        ...
                        className="inline-flex items-center gap-2 px-3 py-1 text-xs font-bold text-emerald-600 bg-white border border-slate-200 rounded-xl hover:bg-emerald-50 transition-colors shadow-sm"
                      >
                ...
                {syllabus.advertencias && syllabus.advertencias.length > 0 && (
                  <div className="bg-amber-50/50 border border-amber-200 rounded-2xl p-4 mt-2">
                    <p className="text-xs text-amber-800 font-bold mb-2 flex items-center gap-2 uppercase tracking-wide">
              ...
              <div className="flex flex-row md:flex-col gap-3 shrink-0">
                <Button
                  ...
                  className="bg-slate-900 hover:bg-black text-white px-6 py-2.5 rounded-2xl flex-1 md:flex-none justify-center gap-2 shadow-sm transition-all"
                >
                ...
                <Button
                  ...
                  className="border-slate-200 text-slate-600 hover:bg-slate-50 rounded-2xl flex-1 md:flex-none justify-center gap-2 transition-all"
                >
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Curso Extraído</p>
                <p className="font-bold text-slate-800">{viewingSyllabus.nombre_curso}</p>
              </div>
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                <p className="text-[10px] font-bold text-slate-400 uppercase mb-1">Periodo Detectado</p>
                <p className="font-bold text-slate-800">{viewingSyllabus.periodo}</p>
              </div>
            </div>

            <div>
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                <FileText className="w-4 h-4" /> Texto del Documento (Primeras 15,000 caracteres)
              </p>
              <div className="bg-slate-900 text-slate-300 p-6 rounded-2xl text-[13px] font-mono leading-relaxed h-[400px] overflow-y-auto shadow-inner border border-slate-800">
        ...
        {selectedSyllabus && (
          <div className="space-y-6">
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">CURSO SELECCIONADO</p>
              <p className="font-bold text-slate-800 text-lg leading-tight">{selectedSyllabus.nombre_curso}</p>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl">
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-indigo-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-indigo-500/20 outline-none"
                  >
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full border border-slate-200 rounded-2xl px-4 py-3 focus:ring-2 focus:ring-indigo-500/20 outline-none text-sm min-h-[100px] resize-none"
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button
                ...
                className={`flex-1 rounded-2xl text-white font-bold ${action === 'approve' ? 'bg-indigo-600 hover:bg-indigo-700 shadow-md shadow-indigo-200' : 'bg-red-600 hover:bg-red-700 shadow-md shadow-red-200'}`}
              >
...
export default PendingSyllabiPage;PendingSyllabiPage;
