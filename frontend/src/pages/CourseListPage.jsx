    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-10" data-tour="student-course-catalog">
      <div className="mb-8 md:flex md:items-end md:justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-2">Explora e inscríbete en los cursos disponibles para este periodo.</p>
        </div>
        
        <div className="mt-4 md:mt-0">
          {currentPeriod ? (
            <div className="inline-flex flex-col items-end">
              <span className="flex items-center gap-1.5 px-3 py-1 bg-indigo-50 text-indigo-700 rounded-lg text-sm font-semibold border border-indigo-100">
            ...
            <div className="inline-flex items-center gap-2 text-sm text-amber-700 bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-100/50">
      ...
      <div className="mb-8 bg-white p-4 border border-slate-200 rounded-2xl shadow-sm flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="w-full md:w-1/2 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors text-sm font-medium"
          />
        </div>
        ...
        <div className="w-full md:w-auto flex flex-col md:flex-row items-center gap-4">
          <div className="relative w-full md:w-48">
            ...
            <select
              ...
              className="w-full pl-9 pr-8 py-2.5 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-colors text-sm appearance-none font-medium text-slate-700"
            >
      ...
      {loading ? (
        ...
        <div className="space-y-8">
          {cycleOrder.map(cycle => {
            ...
            return (
              <div key={cycle} className="scroll-mt-8 bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                {/* Cabecera del Ciclo */}
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-slate-100 bg-white">
                        <th className="py-3 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider w-32">Código</th>
                        <th className="py-3 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider">Asignatura</th>
                        <th className="py-3 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider hidden sm:table-cell">Área / Escuela</th>
                        <th className="py-3 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider text-center w-24">Créditos</th>
                        <th className="py-3 px-6 text-xs font-bold text-slate-400 uppercase tracking-wider text-right w-36">Acción</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {cycleCourses.map(course => (
                        <tr key={course.id_curso} className="hover:bg-slate-50 transition-colors group">
                          <td className="py-4 px-6 align-middle">
                            <span className="font-mono text-xs font-semibold text-slate-600 bg-slate-100 border border-slate-200/60 px-2 py-1 rounded-md">
                              {course.codigo_curso}
                            </span>
                          </td>
                          <td className="py-4 px-6 align-middle">
                            <p className="font-bold text-slate-800 leading-tight group-hover:text-indigo-700 transition-colors">
                              {course.nombre_curso}
                            </p>
                          </td>
                          ...
                          <td className="py-4 px-6 align-middle text-center">
                            <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-indigo-50 text-indigo-700 text-sm font-bold border border-indigo-100">
                              {course.creditos}
                            </span>
                          </td>
